...
| 形 | 2x2 のペンで、行優先に塗っていける形であること |

「形」の条件はポータルの検査を実測して割り出したもので、次の走査と同じ。

> 左上から行優先に見ていき、まだ塗られていない点灯画素に出会ったら、
> その画素を左上とする 2x2 が全点灯していなければならない。
> 全点灯なら、その4画素を「塗った」ことにして先へ進む。
...
- 条件を満たさない箇所は 2x2 を**埋める方向**で直し、描き足した数を表示する
  （元の絵から画素を削らないので、描いた形はそのまま残る）
- 条件を最初から満たしている PNG は、一切変更せずにそのまま送る

#### 描いた形を1画素も変えずに入れる（`--via-editor`）

```sh
ehup app icon --app com.example.myapp --icon icon.png --via-editor
```

PNG を送らず、内蔵の作成ツールを 2x2 ペンで操作して描く。
ツール側の検査は上記のとおり緩いので、アップロード経路では描き足しが要る絵でも、
そのまま通せることが多い。

ペンを置く位置は「全点灯 2x2 の左上」すべて。その和集合は元の絵と一致するため、
描いた形がそのまま再現される。ペンを置く回数は実行時に表示される。

ツール側の検査も満たさない絵の場合は、この経路でも描き足しが入る（数は表示される）。
